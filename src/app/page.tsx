...
// Get domain info from URL or use default
function getDomainInfo(): DomainInfo {
  if (typeof window !== 'undefined') {
    const hostname = window.location.hostname;
    // Remove 'www.' if present and get the domain key
...
export default function Home() {
  ...
  const sendInitialMessage = async () => {
    try {
      ...
      setMessages([
        { 
          text: `Hello! Welcome to the chat. You're interested in ${domain.name}. How can I help you today?`, 
          isUser: false 
        }
      ]);
    } catch (error) {
    ...
    try {
      ...
      setTimeout(() => {
        setMessages(prev => [...prev, { 
          text: 'Message received! I will get back to you soon.', 
          isUser: false 
        }]);
      }, 1000);
    } catch (error) {
